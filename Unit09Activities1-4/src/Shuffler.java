...
public class Shuffler {
	 ...
	 */
	private static final int VALUE_COUNT = 8;   //must be even number for perfect shuffle to work properly
	...
	public static int[] perfectShuffle(int[] values) {
		...
		{
			output[2*k] = firstHalf[k];
			output[2*k+1] = secondHalf[k];
		}
		return output;
	...
	public static int[] selectionShuffle(int[] values) {
		...
		while(tester)
		{
			...
			if(tempValues[randInt] != -1)
			{
				output[outputCounter] = tempValues[randInt];
				tempValues[randInt] = -1;
				outputCounter ++;
			}
