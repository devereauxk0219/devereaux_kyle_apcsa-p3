...
public class Deck {
	...
	public void shuffle() {
		...
		ArrayList<Card> tempValues = cards;
		...
		while(tester)
		{
			...
			if(tempValues.get(randInt) != null)
			{
				output.add(tempValues.get(randInt));
				tempValues.set(randInt, null);
			}
			
			tester = false;
			for (int i = 0; i<tempValues.size(); i++)
			{
				if(tempValues.get(i) != null)
				{
					tester = true;
				}
			}
	...
	public Card deal() {
		...
		if (size()>0)
		{
			...
			ArrayList<Card> cardsReplace = new ArrayList<Card>();
			for (int i = 0; i<size()-1; i++)
			{
				cardsReplace.add(cards.get(i));
			}
			cards = cardsReplace;
			...
			ArrayList<Card> dealtNew = new ArrayList<Card>();
			for (int j = 0; j<dealt.size(); j++)
			{
				dealtNew.add(dealt.get(j));
			}
			dealtNew.add(output);
			dealt = dealtNew;
